...
def get_ollama_host() -> str:
    """
    RDP deployment default:
    - when running directly on the RDP box, localhost is usually correct
    - when calling across the LAN, set OLLAMA_HOST explicitly in .env
    """
    return opt_env("OLLAMA_HOST", "http://10.4.3.45:11434").rstrip("/")
...
def run_ollama_structured(
    ...
    for model in models:
        ...
        for attempt in range(1, attempts_per_model + 1):
            ...
            meta: dict = {}
            try:
                LOG.info("Ollama request started | task=%s | model=%s | attempt=%s", task, model, attempt)
                ...
                msg = f"{model} attempt {attempt}: {str(exc)}"
                ...
                if enable_repair_prompt and raw_text:
                    try:
                        ...
                        for repair_model in get_task_models("repair"):
                            ...
                            return repaired, f"{repair_model}:repair", merged_meta
                    except Exception as repair_exc:
                        errors.append(f"{model} repair after attempt {attempt}: {str(repair_exc)}")
...
def ollama_configured() -> bool:
    return bool(opt_env("OLLAMA_HOST", ""))
...
def generate_with_groq(article: ScrapedArticle) -> Tuple[dict, str]:
    ...
    ok, reason = is_summary_substantive(parsed)
    if not ok:
        raise RuntimeError(f"Groq summary was structurally valid but not substantive from model {model}: {reason}")
    usage = meta.get("usage") if isinstance(meta.get("usage"), dict) else {}
    article.warnings.append(
        "Groq meta: "
...
def generate_summary(article: ScrapedArticle, backend: str) -> Tuple[dict, str]:
    requested_backend = select_backend(backend)
    if requested_backend == "rule":
        return build_rule_based_summary(article), "rule_based"
    attempted_ai = False
    for option in summary_backend_order(requested_backend):
        if option == "groq":
            attempted_ai = True
            try:
...
def maybe_filter_with_llm_relevance(
    ...
    return filtered
# ============================================================
# OLLAMA RDP FIRST-STEP SOURCE PROBE
# ============================================================
def build_ollama_source_probe_prompt(
    ...
    payload = {
        "task": "Before any scraping starts, check whether you already have usable analyst-review style knowledge or resource candidates from these requested sites for the requested market scope and date.",
    ...
    return (
        "You are the first-step resource probe for an analyst-brief pipeline running against an Ollama server on the RDP machine.\n"
        "Your job is NOT to browse.\n"
        "Your job is to say whether the model side already has any usable structured analyst-review items from the requested sources before the normal scraping pipeline begins.\n"
        "If not, explicitly tell the caller to fall back to scraping by returning status=fallback_to_scraping and items=[].\n"
        "Return VALID JSON ONLY.\n\n"
        f"PAYLOAD:\n{json.dumps(payload, ensure_ascii=False, indent=2)}"
    )
...
def build_sharepoint_fields_from_probe_item(item: OllamaSourceProbeItem, run_id: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "GenerationMode": "ollama_rdp_source_probe",
        "Warnings": truncate(
            "\n".join([
                "First-step Ollama RDP source probe used before scraping.",
        ...
        "RunId": run_id,
        "UniqueKey": stable_key(item.source_url or "", item.title, published_at, item.source_name, "ollama_rdp_probe"),
    }
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Analyst brief generator v9 orchestrator (RDP Ollama probe + source-profile dynamic scraping + SharePoint + email alerts).")
    ...
    llm_features_needed = selected_backend in {"groq", "ollama"} or env_bool("USE_LLM_RELEVANCE_FILTER", False)
    ...
    if ollama_runtime_needed:
        ...
        try:
            ...
            append_run_log(run_id, {"stage": "ollama_preflight", "host": host, "models": models, "ts": utc_iso_now()})
        except Exception as exc:
            LOG.warning("Ollama preflight failed | host=%s | error=%s", host, exc)
            append_run_log(run_id, {"stage": "ollama_preflight_failed", "host": host, "error": str(exc), "ts": utc_iso_now()})

    generated_rows: List[Dict[str, Any]] = []
    probe_info: Dict[str, Any] = {}
    if selected_backend == "ollama" and env_bool("ENABLE_OLLAMA_SOURCE_PROBE", True):
        try:
